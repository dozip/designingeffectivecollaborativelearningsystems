"""Option-D U-shaped split-learning TimeMixer backend with dynamic clients.

Split implemented here:
    Client: scaling + multiscale downsampling + original-style input embedding only
    Server: dynamic aggregation, personalized redistribution, full TimeMixer PDM/FMM
    Client: receives its own forecast back and inverse-scales locally

No additional privacy projection is used. The client sends embedded multiscale states,
not raw demand values. This is the server-heavy split: most representation learning
happens on the server while raw sequences remain on the client side.
...
# ---------------------------------------------------------------------------
# Client: only original-style embedding part
# ---------------------------------------------------------------------------


class TimeMixerEmbeddingClient(nn.Module):
    """Client-side module for Option D.

    Responsibilities:
    - create multiscale observations from local private demand history
    - apply trainable input embeddings for each scale
    - send embedded states to the server

    There is no extra privacy projection here. The only neural transformation
    before the split boundary is the TimeMixer-style input embedding.
    """
    ...
    def __init__(
        ...
        # This is the client-side equivalent of TimeMixer's early value embedding.
        # For univariate demand, each scalar at each time step becomes a d_model vector.
...
# ---------------------------------------------------------------------------
# Server: aggregation + redistribution + full TimeMixer processing
# ---------------------------------------------------------------------------


class OptionDTimeMixerServer(nn.Module):
    """Server-side TimeMixer for the Option-D split.

    Server entry aggregation:
        For every scale, stack all client/channel embeddings as [B, N, L, D].
        A learned gate produces a weighted global representation [B, L, D].

    Server redistribution before TimeMixer:
        Each channel receives a personalized global-aware state by combining
        its local embedding with the global aggregated state:
            personalized = f_server([local_embedding, global_embedding])

    Server TimeMixer processing:
        The server then applies PDM and FMM to each personalized channel state:
            decomposition -> seasonal fine-to-coarse mixing
                          -> trend coarse-to-fine mixing
                          -> future multipredictor mixing

    Return path:
        A forecast is returned for every input channel. The client can inverse-
        scale its own forecast locally.
    """
    ...
    def __init__(
        ...
        # server-side personalized representation before PDM.
        ...
        # FMM: scale-specific predictors with learned scale ensemble.
    ...
    def _redistribute_to_channels(
        ...
        for channel_embeds in embedded_by_channel:
            ...
            for scale_id, local_state in enumerate(channel_embeds):
                ...
                # Residual keeps the channel's own embedded information present
                # after the global server context is injected.
    ...
    def _process_one_channel(self, personalized_scales: Sequence[torch.Tensor]) -> torch.Tensor:
        """Run server-side PDM/FMM for one channel and return [B, horizon, output_dim]."""
        ...
        # FMM: one predictor per scale, then learned scale ensemble.
    ...
    def forward(
        ...
        """Return one server prediction per channel.
        ...
        Returns:
            outputs_by_channel: list of [B, horizon, output_dim]
            aux: lightweight diagnostics about number of processed channels/scales
        """
...
@register_backend("split_timemixer_option_d")
class TimeMixerOptionDSplitBackend(ForecastingBackend):
    """Server-heavy U-shaped split TimeMixer backend.

    Client side:
        scaling, multiscale downsampling, trainable embeddings.

    Server side:
        dynamic aggregation, redistribution, decomposition, seasonal mixing,
        trend mixing, and future prediction.

    Return path:
        forecasts are returned to the clients/agents and inverse-scaled locally.
    """
    ...
    def collaborative_predict(self, level_agents, demand_t, sc_agent_list, t) -> List[List[float]]:
        ...
        with torch.no_grad():
            ...
            for channel_idx, (agent_idx, retailer_idx) in enumerate(channel_to_agent_retailer):
                ...
                value_scaled = outputs_by_channel[channel_idx][:, -1, 0].item()
...
def _train_split_timemixer_option_d(simulation, market, supply_chain, sc_agent_list, cfg):
    ...
    logger.info("Option-D client embedding params total: %d", client_params)
    ...
    for epoch in range(epochs):
        ...
        for batches in zip(*trainloaders):
            ...
            # Client forward: only embeddings cross the split boundary.
            embedded_original_by_channel: List[List[torch.Tensor]] = []
            embedded_detached_by_channel: List[List[torch.Tensor]] = []
            for channel_idx, spec in enumerate(channel_specs):
                client = agent_client_models[spec.agent_idx][spec.retailer_idx]
                embedded_original = client(features_by_channel[channel_idx])
                embedded_detached = [
                    tensor.detach().requires_grad_(True)
                    for tensor in embedded_original
                ]
                embedded_original_by_channel.append(embedded_original)
                embedded_detached_by_channel.append(embedded_detached)

            # Server forward: aggregate -> distribute -> full TimeMixer -> forecast.
            outputs_by_channel, _aux = server_model(embedded_detached_by_channel)

            # Loss: individual per channel or aggregated per agent.
            ...
            # Backward through server. Then manually pass gradients through the
            # split boundary into the original client embeddings.
            total_loss.backward(retain_graph=True)

            flat_pairs: List[Tuple[torch.Tensor, Optional[torch.Tensor]]] = []
            for original_scales, detached_scales in zip(
                embedded_original_by_channel, embedded_detached_by_channel
            ):
                for original, detached in zip(original_scales, detached_scales):
                    flat_pairs.append((original, detached.grad))

            for pair_idx, (original, grad) in enumerate(flat_pairs):
                if grad is not None:
                    original.backward(grad, retain_graph=(pair_idx < len(flat_pairs) - 1))
        ...
        with torch.no_grad():
            ...
            for channel_idx, spec in enumerate(channel_specs):
                ...
                client = agent_client_models[spec.agent_idx][spec.retailer_idx]
                embedded_by_channel.append(client(x_val))
                targets_by_channel.append(y_val)

            outputs_by_channel, _aux = server_model(embedded_by_channel)
