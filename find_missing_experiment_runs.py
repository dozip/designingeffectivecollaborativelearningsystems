...
2. rerun_configs/rerun_run_<ID>.yaml
   - Runner-compatible YAML for one run ID
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Recovery index YAML. Default: missing_experiments.yaml",
    )
    parser.add_argument(
        "--rerun-dir",
        type=Path,
        default=Path("rerun_configs"),
        help="Directory for executable per-run YAML files.",
    )
    parser.add_argument(
        "--run-ids",
        type=parse_run_ids,
    ...
    parser.add_argument(
        "--no-rerun-configs",
        action="store_true",
        help="Only write the recovery index; do not create per-run YAML files.",
    )
    parser.add_argument(
        "--strict-names",
        action="store_true",
...
def main() -> int:
    ...
    output_path = (
        ...
        else (parent / output_arg).resolve()
    )
    rerun_dir = (
        rerun_arg.resolve()
        if rerun_arg.is_absolute()
        else (parent / rerun_arg).resolve()
    )
    ...
    unexpected_experiment_dirs = sorted(
        ...
        and child.name not in {output_path.name, rerun_dir.name}
    )
    ...
    elif not args.no_rerun_configs:
